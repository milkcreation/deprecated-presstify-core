...
abstract class tiFY_Plugin{
	...
	/** == Récupération du chemin vers le repertoire == **/
	public static function get_name( $plugin_class ){		
	...
	public static function get_config( $plugin_class ){
		...
		if( ! empty( $tiFy->params['plugins'][self::get_name( $plugin_class )] ) ) :
			
			return $tiFy->params['plugins'][self::get_name( $plugin_class )];
		else :
			return $defaults;
		endif;					
	}
