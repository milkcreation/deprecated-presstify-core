<?php
class tiFy_Wistify_Campaign_AdminEdit extends tiFy_AdminView_Edit_Form{
	/* = ARGUMENTS = */	
	public	$current_step;
	...
	public function __construct( tiFy_Wistify_Master $master ){
		...
		// Paramétrage
		/// Environnement
		$this->item_action_base_link = add_query_arg( array( 'page' => $this->master->menu_slug['campaign'], 'view' => 'edit_form' ), admin_url( '/admin.php' ) );
	...
	?>	
		...
		<?php foreach( range( 1, 5, 1 ) as $step ) :?>
			...
			<?php if( ( $step <= $this->item->campaign_step ) && ( $step != $this->current_step ) ) :?>
				<a href="<?php echo add_query_arg( array( $this->item_request => $this->item_id, 'step' => $step ), $this->item_action_base_link );?>"><?php echo $step_txt;?></a>
			<?php else :?>
	...
	?>
		...
		<?php if( isset( $this->item->campaign_recipients['wystify_subscriber'] ) ) :?>
			<?php foreach( (array) $this->item->campaign_recipients['wystify_subscriber'] as $recipient ) : if( ! $this->master->db_subscriber->get_item_by_id( $recipient ) ) continue; ?>
				<li data-numbers="1">
					<span class="ico">
						<i class="fa fa-user"></i>
						<i class="badge wisti-logo"></i>
					</span>
					<span class="label"><?php echo $this->master->db_subscriber->get_item_var_by_id( $recipient, 'email' );?></span>
		...
		<?php if( isset( $this->item->campaign_recipients['wystify_mailing_list'] ) ) :?>
			<?php foreach( (array) $this->item->campaign_recipients['wystify_mailing_list'] as $list_id ) : $numbers = $this->master->db_subscriber->count_items( array( 'list_id' => $list_id, 'status' => 'registred', 'active' => 1 ) );?>
				<li data-numbers="<?php echo $numbers;?>">
					<span class="ico">
						<i class="fa fa-group"></i>
						<i class="badge wisti-logo"></i>
					</span>
					<span class="label"><?php echo $this->master->db_list->get_item_var_by_id( $list_id, 'title' );?></span>
	...
	public function step_5( ){
		...
		if( isset( $this->item->campaign_recipients['wystify_mailing_list'] ) )
			foreach( $this->item->campaign_recipients['wystify_mailing_list'] as $list_id )
				$total += $this->master->db_subscriber->count_items( array( 'list_id' => $list_id, 'status' => 'registred' ) );
	...
	public function minor_actions(){?>
		...
		<div class="nav">
			...
			<a href="<?php echo add_query_arg( array( 'step' => $this->current_step-1, $this->item_request => $this->item->campaign_id ), $this->item_action_base_link );?>" class="prev button-secondary"><?php _e( 'Étape précédente', 'tify' );?></a>
			<?php endif;?>
			<?php if( $this->item->campaign_step > $this->current_step ) :?>
			<a href="<?php echo add_query_arg( array( 'step' => $this->current_step+1, $this->item_request => $this->item->campaign_id ), $this->item_action_base_link );?>" class="next button-secondary"><?php _e( 'Étape suivante', 'tify' );?></a>
			<?php endif;?>
		</div>
		<?php endif;?>
	<?php
	...
	public function process_bulk_action_edit(){		
		if( ! $this->item_id ) :				
			...
			$item = $this->db->get_item_by_id( (int) $_GET[$this->primary_key] );
			...
			if( ! in_array( $item->campaign_status, array( 'edit', 'ready', 'draft', 'auto-draft' ) ) )
				wp_die( sprintf( __( 'Le statut actuel de la campagne ne permet pas de la modifier.', 'tify' ) ) );
			
			if( ! isset( $_GET['step'] ) ) :
				$sendback = add_query_arg( array( $this->item_request => $this->item_id, 'step' => $item->campaign_step ), $this->item_action_base_link );		
				wp_redirect( $sendback );
				exit;
	...
	public function process_bulk_action_update(){	
		...
		if( isset( $data['campaign_message_options']['from_email'] ) && ! is_email( $data['campaign_message_options']['from_email'] ) ) :
			$sendback = add_query_arg( array( 'message' => 'invalid_format_from_email' ), $sendback );
		elseif( $this->db->insert_item( $data ) ) :					
			$sendback = add_query_arg( array( 'message' => 'updated' ), $sendback );
			if( $data['campaign_status'] === 'send' )
				$sendback = $this->main->list_table_link;		
		endif;					
