			...
			foreach( (array) self::$Section as $name => $args ) :				
				...
				foreach( (array) $args['post_type'] as $pt ) :
					if( ! in_array( $pt, self::$PostTypeTags ) ) 
						continue;
					$join .= " LEFT OUTER JOIN {$wpdb->term_relationships} AS tystr ON ({$wpdb->posts}.ID = tystr.object_id)";
					$join .= " LEFT OUTER JOIN {$wpdb->term_taxonomy} AS tystt ON (tystr.term_taxonomy_id = tystt.term_taxonomy_id  AND tystt.taxonomy = 'tify_search_tag')";
					$join .= " LEFT OUTER JOIN {$wpdb->terms} AS tyst ON (tystt.term_id = tyst.term_id)";
					$groupby = "{$wpdb->posts}.ID";
					break;
				endforeach;					
	...
	private function ParseSectionSearchRequest( &$q )
	{
		...
		foreach ( $q['search_terms'] as $term ) {
			...
			foreach( (array) $q['post_type'] as $pt ) :
				if( ! in_array( $pt, self::$PostTypeTags ) ) 
					continue;
				$searchquery[] = '( ( tyst.name '. $like_op .' "%1$s" ) OR ( tyst.slug '. $like_op .' "%1$s" ) )';
			endforeach;
