...
class Seo extends Plugin
{
	...
	public function __construct()
	{
		...
		add_filter( 'tify_seo_title_is_page_on_front', array( $this, 'tify_seo_title_for_singular' ), 10, 3 );		
		add_filter( 'tify_seo_title_is_single', array( $this, 'tify_seo_title_for_singular' ), 10, 3 );
		add_filter( 'tify_seo_title_is_page', array( $this, 'tify_seo_title_for_singular' ), 10, 3 );		
		add_filter( 'tify_seo_title_is_page_for_posts', array( $this, 'tify_seo_title_for_archive' ), 10, 3 );
		add_filter( 'tify_seo_title_post_type_archive', array( $this, 'tify_seo_title_for_hook_for_archive' ), 10, 3 );
		add_filter( 'tify_seo_desc_is_page_on_front', array( $this, 'tify_seo_desc_for_singular' ), 10, 3 );
		add_filter( 'tify_seo_desc_is_single', array( $this, 'tify_seo_desc_for_singular' ), 10, 3 );
		add_filter( 'tify_seo_desc_is_page', array( $this, 'tify_seo_desc_for_singular' ), 10, 3 );	
		add_filter( 'tify_seo_desc_post_type_archive', array( $this, 'tify_seo_desc_for_post_type_archive' ), 10, 3 );
	}
	...
	{
		// Configuration
		$post_type	 			= ( empty( self::getConfig( 'post_type' ) ) ) ? get_post_types() : self::getConfig( 'post_type' );
		self::$post_type 		= array_diff( $post_type, array( 'attachment', 'revision', 'nav_menu_item' ) );
		self::$sep				= self::getConfig( 'sep' );
		self::$append_blogname	= self::getConfig( 'append_blogname' );
		self::$append_sitedesc	= self::getConfig( 'append_sitedesc' );
		...
		foreach( (array) self::$post_type as $post_type )
			tify_taboox_register_node( 
				...
				array( 
					...
					'cb' 		=> 'tiFy\Plugins\Seo\Taboox\Admin\Post\MetaTag\MetaTag' 
				) 
			);
	}
	...
	final public function tify_options_register_node(){	
		...
		tify_options_register_node(
			array(
				...
				'cb' 		=> 'tiFy\Plugins\Seo\Taboox\Admin\Option\OpenGraph',
				'order'		=> 1
			)
		);
		tify_options_register_node(
			array(
				...
				'cb' 		=> 'tiFy\Plugins\Seo\Taboox\Admin\Option\GoogleAnalytics',
				'order'		=> 2
			)
		);
	}
	...
	/** == HOOK FOR ARCHIVE == **/
	final public function tify_seo_title_for_hook_for_archive( $title ) 
	{
		global $post;
	
		if( is_post_type_archive() && ( $hook_id = get_query_var( 'tify_hook_id' ) ) && ( $seo_meta = get_post_meta( $hook_id, '_tify_seo_meta', true ) ) && ! empty( $seo_meta['title'] ) )
			return $seo_meta['title'];
	...
	/** == HOOK FOR ARCHIVE == **/
	final public function tify_seo_desc_for_post_type_archive( $desc ) 
	{
		global $post;
	
		if( is_post_type_archive() && ( $hook_id = get_query_var( 'tify_hook_id' ) ) && ( $seo_meta = get_post_meta( $hook_id, '_tify_seo_meta', true ) ) && ! empty( $seo_meta['desc'] ) )
			return $seo_meta['desc'];
