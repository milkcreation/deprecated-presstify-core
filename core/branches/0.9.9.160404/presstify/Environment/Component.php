...
abstract class Component extends App
{
	...
	/** == Initialisation du nom d'appel du composant == **/
	...
	/** == Initialisation de la configuration du composant == **/
	private function initConfig()
	{
		...
		if( ! empty( $this->Params['components'][$this->Name] ) ) :
			$config = \tiFy\Core\Params::evalPHP( $this->Params['components'][$this->Name] );			
			return self::$Config[$class] = wp_parse_args( $config, self::$DefaultConfig[$class] );	
			
		elseif( preg_match( '/'. preg_quote( 'tiFy\\Components\\', '\\' ) .'/', $this->Name )	&& ! empty( $this->Params['components'][$this->ShortName] ) ) :
			$config = \tiFy\Core\Params::evalPHP( $this->Params['components'][$this->ShortName] );	
			return self::$Config[$class] = wp_parse_args( $config, self::$DefaultConfig[$class] );	
