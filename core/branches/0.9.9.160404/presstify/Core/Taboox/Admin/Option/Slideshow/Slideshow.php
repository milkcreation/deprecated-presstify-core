<?php
namespace tiFy\Core\Taboox\Admin\Option\Slideshow;
...
class Slideshow extends Admin
{
	...
	public function admin_enqueue_scripts()
	{
		...
		wp_register_script( 'tinyMCE', /*includes_url( 'js/tinymce' ). '/tinymce.min.js'*/ '//cdn.tinymce.com/4/tinymce.min.js', array(), ' 4.1.4', true );
		wp_register_script( 'jQuery-tinyMCE', '//cdn.tinymce.com/4/jquery.tinymce.min.js', array( 'tinyMCE', 'jquery' ), true );
	...
	private function item_render( $slide, $index = 0 )
	{
		...
		foreach( $this->args['options']['attrs'] as $attr )
			$output .= apply_filters( "tify_taboox_slideshow_item_{$attr}", ( method_exists( $this, "item_set_{$attr}" ) ? call_user_func_array( array( $this, "item_set_{$attr}" ), array( $name, $index, $slide ) ) : '' ), $name, $index, $slide );
	...
	/* = PARAMÉTRAGE = */
	/** == TITRE == **/
	public function item_set_title( $name, $index, $slide )
	{
	...
	/** == LIEN == **/
	public function item_set_link( $name, $index, $slide )
	{
		$output = "\n\t\t<div class=\"link\">";
		$output .= "\n\t\t\t<h3>". __( 'Lien', 'tify' ) ."</h3>";
		$output .= "\n\t\t\t<label>";
	...
	/** == LÉGENDE == **/
	public function item_set_caption( $name, $index, $slide )
	{
	...
	/** == PLANNING == **/
	public function item_set_planning( $name, $index, $slide )
	{
		$output = "\n\t\t<div class=\"planning\">";
		$output .= "\n\t\t\t<h3>Programmation</h3>";
