...
class EditForm
{
	...
	{
		$this->query = $this->Entity->getDb()->query();

		if( $items = $this->query->query( array( $this->Entity->getDb()->Primary => $this->current_item() ) ) )
	...
	public function get_fields() 
	{
		...
		foreach( (array) $this->Entity->getDb()->ColNames as $name )
			$f[$name] = $name;
	...
	{		
		...
		elseif( ! $this->Entity->getDb()->select()->row_by_id( $item_id ) )
			wp_die( __( 'Vous tentez de modifier un contenu qui n’existe pas. Peut-être a-t-il été supprimé ?!', 'tify' ) );

		// Traitement des actions
		if( ! $this->current_item() ) :
			wp_safe_redirect( add_query_arg( $this->Entity->getDb()->Primary, $item_id ) );
			exit;
	...
	{
		return $this->Entity->getDb()->handle()->create( wp_parse_args( $this->item_defaults, array( $this->Entity->getDb()->Primary => 0 ) ) );
	}
	
	/** == Éxecution de l'action - mise à jour == **/
	protected function process_bulk_action_update()
	{		
		check_admin_referer( $this->Entity->getDb()->ID . $this->current_action() . $this->current_item() );
				
		$data = $this->parse_postdata( $_POST );
		
		$sendback = remove_query_arg( array( 'action', 'action2' ), wp_get_referer() );
		$sendback = add_query_arg( array( $this->Entity->getDb()->Primary => $this->current_item() ), $sendback );
		if( is_wp_error( $data ) ) :
			$sendback = add_query_arg( array( 'message' => $data->get_error_code() ), $sendback );	
		else :		 
			$this->Entity->getDb()->handle()->record( $data );			
	...
	protected function process_bulk_action_trash()
	{
		...
		if( $this->Entity->getDb()->hasMeta() && ( $original_status = $this->Entity->getDb()->select()->cell_by_id( $this->item_id, 'status' ) ) )
			$this->Entity->getDb()->meta()->update( $this->item_id, '_trash_meta_status', $original_status );					
		/// Modification du statut
		$this->Entity->getDb()->handle()->update( $this->item_id, array( 'status' => 'trash' ) );
	...
	public function field_default( $item, $field_name )
	{
		...
		if( ! isset( $item->{$field_name} ) )
			return;
		if( $field_name === $this->Entity->getDb()->Primary )
			return "#{$item->{$field_name}}";		
			
		$col_type = strtoupper( $this->Entity->getDb()->getColAttr( $field_name, 'type' ) );
	...
	?>
		<div id="submitdiv" class="tify_submitdiv">
			<?php wp_nonce_field( $this->Entity->getDb()->ID  .'update'. $this->item->{$this->Entity->getDb()->Primary} ); ?>
			...
			<input type="hidden" id="<?php echo $this->Entity->getDb()->Primary;?>" name="<?php echo $this->Entity->getDb()->Primary;?>" value="<?php echo $this->item->{$this->Entity->getDb()->Primary};?>" />
			<h3 class="hndle">
	...
	?>
		<div class="wrap">
			<h2>
				...
				<?php if( $this->Entity->getAdminView( 'base_url' ) ) : ?>
					<a class="add-new-h2" href="<?php echo $this->Entity->getAdminView( 'base_url' );?>"><?php echo $this->Entity->getLabel( 'new_item' );?></a>
				<?php endif;?>
			</h2>
