...
class AdminUI extends Component
{
	...
	{
		foreach( (array) self::getConfig( 'remove_menu' ) as $menu => $disabled ) :
			if( $disabled ) :
				switch( $menu ) :
					default :
	...
	{
		foreach( (array) self::getConfig( 'unregister_widget' ) as $widget => $disabled ) :
			if( $disabled ) :
				switch( $widget ) :
					default :
	...
	{
		foreach( (array) self::getConfig( 'remove_dashboard_meta_box' ) as $meta_box => $disabled ) :
			if( $disabled ) :
	...
	private function remove_meta_box_post_type()
	{
		foreach( array_keys(self::getConfig() ) as $config ) :
			...
			foreach( (array) self::getConfig( $config ) as $meta_box => $context ) :
				remove_meta_box( $meta_box, $post_type, ( is_bool( $context ) ? 'normal' : $context ) );
	...
	private function remove_admin_bar_menu()
	{
		...
		foreach( (array) self::getConfig( 'remove_admin_bar_menu' ) as $admin_bar_node => $disabled ) :
			if( $disabled ) :
