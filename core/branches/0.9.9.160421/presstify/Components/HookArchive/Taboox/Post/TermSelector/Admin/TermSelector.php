...
class TermSelector extends Admin
{
	...
	?>
		<h3><?php _e( 'Choix de(s) catégorie(s) d\'affichage du contenu', 'tify' );?></h3>	
		<?php $this->Nested->form( $post ); ?>
		<h3><?php _e( 'Choix de la catégorie du permalien (catégorie principale)', 'tify' );?></h3>
		<?php if( $terms = get_terms( array( 'taxonomy' => $this->args['taxonomy'], 'hide_empty' => false ) ) ) :?>
