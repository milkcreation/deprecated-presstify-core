			...
			foreach( (array) self::getConfig( $env ) as $type => $custom_columns ) :
				foreach( (array) $custom_columns as $Classname => $args ) :
					...
					if( \class_exists( $Classname ) ) :						
						...
						$Classname = "\\tiFy\\Components\\CustomColumns\\". ucfirst( $env ) ."\\". $Classname ."\\". $Classname;
						if( ! \class_exists( $Classname ) )
							continue;
						self::$Factories[$env][$type][] = new $Classname( $args );	
					endif;
	...
	{			
		...
		switch( $current_screen->base ) :
			default:
				...
				break;
			case 'edit' :
				if( ! isset( self::$Factories['post'][$current_screen->post_type] ) )
					return;
				foreach( (array) self::$Factories['post'][$current_screen->post_type] as $FactoryClass ) :
