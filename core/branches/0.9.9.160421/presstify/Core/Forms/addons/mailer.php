...
class tiFy_Forms_Addon_Mailer extends tiFy_Forms_Addon{
	...
	public function cb_handle_before_redirect( &$parsed_request, $original_request ){
		...
		if( ( $options = $this->master->addons->get_form_option( 'confirmation', $this->ID ) ) && $options['send'] ) :
			// Préparation du mail
			if( $options = $this->parse_options( $options, $parsed_request['fields'] ) )
				new tiFy\Lib\Mailer\Mailer( $options );	
		endif;
		
		// Envoi du message de notification
		if( ( $options = $this->master->addons->get_form_option( 'notification', $this->ID ) ) && $options['send'] ) :	
			// Préparation du mail
			if( $options = $this->parse_options( $options, $parsed_request['fields'] ) )	
				new tiFy\Lib\Mailer\Mailer( $options );				
		endif; 
	...
	protected function parse_options( $options, $fields ){
		...
		$auto = ( $this->master->addons->get_form_option( 'debug', $this->ID ) ) ? 'debug' : 'send';		
	...
	protected function html_prepare( $subject, $fields ){
		$output  = '';
		...
		foreach( $fields as $field ) :
			...
			if( $field['type'] == 'file' ) continue;
			if( $field['add-ons'][$this->ID]['ignore'] ) continue;	
	...
	protected function parse_contact( $contact, $fields ){
		$output = array();
		if( is_array( $contact ) ) :
