...
class Query
{
	...
	{	
		...
		$join .= " INNER JOIN ". Events::GetDb()->Name ." as tify_events ON ( $wpdb->posts.ID = tify_events.event_post_id )";  	
		
		if( ! $query->is_singular() ) :
			if( $show === 'uniq' ) :
				$inner_where  = "SELECT MIN( event_start_datetime ) FROM ". Events::GetDb()->Name ." WHERE 1";
	...
	public static function PostGetEvents( $post = 0, $args = array() )
	{
		...
		return \tiFy\Plugins\Events\Events::GetDb()->select()->rows( $args );
	}
	...
	{
		return \tiFy\Plugins\Events\Events::GetDb()->select()->row_by_id( $event_id );
	}
