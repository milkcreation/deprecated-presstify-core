...
class Screen extends App
{
	...
	protected $SetAttrs		= array( 'Box', 'Nodes', 'Forms' );
	...
	{
		// Définition de l'ID de l'écran courant	
		$this->ID = $screen->id;
	}
	...
	public function nodes_content_render( $depth = 0, $parent = null ){		
		...
		foreach( $nodes as $id ) :	
			...
			if( empty( $node['cb'] ) ) :
				...
				elseif( is_callable( array( $this->Forms[$id], 'form' ) ) ) :			
					ob_start();
					call_user_func_array( array( $this->Forms[$id], 'form' ), $this->box_render_args );
					$output .= ob_get_clean();	 
				endif;
				$output .= 		"</div>";
				$output .= 	"</div>";
