...
class Fileshare extends Helpers
{
	...
	public static function Display( $post_id = null, $args = array(), $echo = true )
	{
		...
		foreach( (array) $files as $file_id ) :
			...
			$output_item .= "\n\t<a href=\"" . tify_upload_url( $file_id, array( 'post_id' => $post_id, 'fileshare_name' => $args['name'] ) ) . "\" class=\"fileshare_link clearfix\"  title=\"" . __( 'Télécharger le fichier', 'tify' ) ."\">";
	...
	final public function tify_upload_register()
	{
		...
		if( ! $attachment_ids = \get_post_meta( (int) $_REQUEST['post_id'], (string) $_REQUEST['fileshare_name'] ) )
			return;

		foreach( $attachment_ids as $attachment_id )
			tify_upload_register( $attachment_id );
	}
