...
final class HookArchive extends Component
{
	...
	// Liste des type autorisés
	private static $AllowedTypes		= array( 'post', 'taxonomy' );
	...
	public function __construct( )
	{
		...
		foreach( (array) self::getConfig('hooks') as $type => $hooks ) :
		...
		foreach( self::$AllowedTypes as $type ) :
			if( isset( self::$Hooks[$type] ) ) :
				foreach( (array) self::$Hooks[$type] as $archive => $Class ) :
					if( $Class->hasAdmin() ) :
						$this->Admin = true;
						break 2;
					endif;
	...
	{
		$defaults = array(
			// (requis) Type d'accroche : post (par défaut) | taxonomy
			'type'						=> 'post',
		...
		switch( $args['type'] ) :
			case 'post' :
				self::$Hooks[$args['type']][$args['archive']] 	= new Post( $args );
				break;
			case 'taxonomy' :
				self::$Hooks[$args['type']][$args['archive']]	= new Taxonomy( $args );
				break;
		endswitch;
	}
	...
	public static function GetPostHook( $post = null, $permalink = true )
	{
		...
		foreach( self::$AllowedTypes as $type ) :
			switch( $type ) :
				case 'post' :
					if( $_hooks = self::GetPostTypeHooks( $post_type, $permalink, 'post' ) ) :
						$hooks += $_hooks;
					endif;
					break;
				case 'taxonomy' :
	...
	public static function GetPostTypeHooks( $post_type, $permalink = true, $object_type = null )
	{
		// Bypass
		if( ! self::$Hooks )
			return;
		...
		if( ! $object_type )
			$object_type = self::$AllowedTypes;
		if( is_string( $object_type ) )
			$object_type = array_map( 'trim', explode( ',', $object_type ) );
			
		$hooks = array();
		foreach( $object_type as $type ) :
			foreach( self::$Hooks[$type] as $archive => $Class ) :	
				if( $permalink && ! $Class->GetOption( 'rewrite' ) )
					continue;				
				switch( $type ) :
					case 'post' :
