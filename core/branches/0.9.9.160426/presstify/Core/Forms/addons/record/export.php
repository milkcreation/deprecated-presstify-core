...
class tiFy_Forms_Addon_RecordExport{
	...
	function wp_ajax_export(){
		// Bypass
		if( ! $this->main->master->forms->set_current( (int) $_REQUEST['form_id'] ) )
			return;
	...
	function render(){
		...
		<form id="tify_forms-export_form" method="post" action="">
			...
			<p>
				...
				<a href="<?php echo add_query_arg( array( 'file_upload_url' => $this->main->master->dirs->uri( 'export' ) .'/'. $filename, '_wpnonce' => wp_create_nonce( "tify_forms_record_export-" . $filename ) ), admin_url() );?>" id="download-csv" class="button-secondary" style="margin-left:10px; display:none;">
