...
class EditForm extends tiFy_EditForm
{
	...
	{		
		...
		wp_enqueue_style( 'tify_Emailing_Admin_MailingList_EditForm', $this->Url .'/EditForm.css', array( ), '150406' );	
	}
	...
	public function parse_postdata( $data )
	{
		...
		// Vérification
		if( ! empty( $data['list_title'] ) && $this->View->getDb()->select()->cell( 'title', array( 'exclude' => $data['list_id'], 'title' => $data['list_title'] ) ) ) 
			...
			<a href="<?php echo wp_nonce_url( 
		        					add_query_arg( 
	        							array( 
	        								'page' 				=> $_REQUEST['page'], 
	        								'action' 			=> 'trash', 
	        								$this->primary_key	=> $this->item->list_id
										),
										admin_url( 'admin.php' ) 
									),
									'wistify_mailing_list_trash_'. $this->item->list_id 
