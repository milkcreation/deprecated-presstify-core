...
class tiFy_Contest_Participation_AdminListTable extends tiFy_AdminView_List_Table{
	...
	public function column_recipe( $item ){
		...
		if( $recipe_image = $this->db->get_item_meta( $item->part_id, 'recipe_image' ) ) :
			$img = "<img src=\"". tiFy\Lib\Utils::get_context_img_src( $recipe_image['upload_dir']['path'] . $recipe_image['name'], 60, 60, true ) ."\" width=\"60\" height=\"60\" style=\"width:60px;height:60px;object-fit:cover;margin-right:5px;float:left;\"/>";
		endif;
	...
	public function column_user_id( $item ){
		...
		if( ( $bonus_image = $this->db->get_item_meta( $item->part_id, 'bonus_image' ) ) )
			if( $src = tiFy\Lib\Utils::get_context_img_src( $bonus_image['upload_dir']['path'] . $bonus_image['name'], 50, 50, true ) )
