			...
			case 'taxonomy' :
				...
				foreach( (array) $terms as $n => $term ) : 
					if( false === ( ( isset( $exists[$term->term_id] ) ) ? (bool) $exists[$term->term_id]['edit'] : (bool) $this->args['options']['edit'] ) )
						continue;
					$output .= "<input type=\"hidden\" name=\"tify_hook_". $this->args['obj'] ."_". $this->args['archive'] ."[$n][term]\" value=\"{$term->term_id}\" />";
					$output .= 	"<table class=\"form-table\">";
					$output .=		"<tbody>";
					$output .= 			"<tr>";
					...
					elseif( ! $post_types = get_taxonomy( $this->args['archive'] )->object_type ) :					
						...
						foreach( (array) $post_types as $post_type ) :
							...
							$output .= "<li><label><input type=\"checkbox\" name=\"tify_hook_". $this->args['obj'] ."_". $this->args['archive'] ."[$n][permalink][]\" value=\"{$post_type}\" ". checked( (bool) $checked, true, false )." autocomplete=\"off\">". $post_type_object->label ."</label></li>";
						endforeach;
						if( $has_post_type ) :
							$output .= "</ul>";
						endif;
