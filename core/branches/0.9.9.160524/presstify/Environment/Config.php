...
abstract class Config extends App
{
	...
	protected function initSchema()
	{
		...
		if( $this->ClassShortName === 'WebService' ) :
			if( ! empty( $this->Params[$this->SubDir][$this->ClassName]['schema'] ) ) :
				$schema = wp_parse_args( $this->Params[$this->SubDir][$this->ClassName]['schema'], $schema ); 
			elseif( preg_match( '/'. preg_quote( $this->Namespace, '\\' ) .'/', $this->ClassName )	&& ! empty( $this->Params[$this->SubDir][$this->ClassShortName]['schema'] ) ) :
				$schema = wp_parse_args( $this->Params[$this->SubDir][$this->ClassShortName]['schema'], $schema ); 
			endif;
		endif;
