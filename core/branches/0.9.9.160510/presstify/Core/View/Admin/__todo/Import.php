...
class Import
{
	...
	final public function _admin_init(){
		...
		add_action( 'wp_ajax_tify_adminview_import_download_sample_'. $this->table_id, array( $this, 'wp_ajax_download_sample' ) );
		add_action( 'wp_ajax_tify_adminview_import_upload_'. $this->table_id, array( $this, 'wp_ajax_upload' ) );
		add_action( 'wp_ajax_tify_adminview_import_handle_'. $this->table_id, array( $this, 'wp_ajax_import' ) );
	...
	{	
		...
		wp_enqueue_style( 'tiFy_AdminView-import', $uri .'/css/import.css', array(), 150607 );
		wp_enqueue_script( 'tiFy_AdminView-import', $uri .'/js/import.js', array( 'jquery', 'tify-fixed_submitdiv' ), 150607 );	
	...
	private function _parse_column_map()
	{
		if( $this->column_map ) :
			...
			foreach( $this->table->ColNames as $col_name )
	...
	public function wp_ajax_download_sample()
	{
		...
		if( ! empty( $this->sample['rows'] ) ) :
			...
			global $wpdb;
			foreach( $wpdb->get_col( "DESC {$this->table_name}", 0 ) as $column )
				$rows[0][] = $column;
			foreach( range( 0, 4, 1 ) as $i )
				$rows[$i+1] = $wpdb->get_row( "SELECT * FROM {$this->table_name}", ARRAY_A, $i );
		endif;
		reset( $rows );
	...
	{
		$this->list_table = new tiFy_AdminView_ImportListTable( $this );
	}
	...
	{		
		...
		foreach( (array) $this->get_file_datas( (int) $this->header ) as $row => $datas ) :
			...
			$tify_adminview_import_result = $this->table_id .'_tify_adminview_import_result';
	...
	private function import_row( $row, $datas )
	{
		...
		if( $item_id = $this->check_item_exists( $row, $item ) )
			$item->{$this->primary_key} = $item_id;
		
		$item = $this->parse_importdata( $item );
		
		if( $item_id = $this->table->handle()->create( (array) $item ) ) :		
	...
	private function check_item_exists( $row, $item )
	{
		...
		return $this->table->select->id( $query_args );
	}
	...
	?>
		<div style="position:relative;padding:20px 0 10px;">
			...
			<form id="tify_adminview_import-uploadfile" method="post" action="" enctype="multipart/form-data">				
				<input id="tify_adminview_import-id" type="hidden" name="tify_adminview_import-id" value="<?php echo $this->table_id;?>">
	...
	?>
		<div class="wrap">
			<h2>
				...
				<a id="tify_adminview_import-download_sample" class="add-new-h2" href="<?php echo esc_url( add_query_arg( array( 'action' => 'tify_adminview_import_download_sample_'. $this->Entity->getID() ), admin_url( 'admin-ajax.php') ) );?>">
...
/* = LISTE = */
class tiFy_AdminView_ImportListTable extends ListTable
{
	/* = ARGUMENTS = */	
	public 	// Contrôleur
			$main;
	...
	/* = CONSTRUCTEUR = */	
	public function __construct( Import $main )
	{			
