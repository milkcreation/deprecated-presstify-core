...
class ListTable extends tiFy_ListTable
{
	...
	{		
		...
		$this->views = array(
			'any'		=> array(
				...
				'count_query_args'	=> array( 'status' => array( 'edit', 'preparing', 'ready', 'send', 'forwarded' ) )
			),
			'edit'			=> array( 
		...
		if( $this->status === 'any' )
			$this->prepare_query_args = array( 'status' => array( 'edit', 'preparing', 'ready', 'send', 'forwarded' ) );
		
		// Mise en file des scripts	
		wp_enqueue_style( 'tiFy_Emailing_Admin_Campaign_ListTable', $this->Url .'/ListTable.css', array( ), '151019' );
	}
	...
	public function handle_row_actions( $item, $column_name, $primary )
	{
		...
		elseif( $item->campaign_status === 'send' ) :
			...
			$actions['cancel'] = "<a href=\"".
									$this->get_single_action_uri( $item->campaign_id, 'cancel', array( '_wp_http_referer' => urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ) ) ) 
	...
	public function process_bulk_action_duplicate()
	{
		...
		if( ! $c = $this->View->getDb()->select()->row_by_id( $item_id ) )
			return;
		...
		$this->View->getDb()->handle()->create( $args );
	...
	public function process_bulk_action_cancel()
	{
		...
		$this->View->getDb()->handle()->update( $item_id, array( 'status' => 'ready' ) );
	...
	{
		return sprintf( '<input type="checkbox" name="%1$s[]" value="%2$s" /><div class="locked-indicator"></div>', $this->_args['singular'], $item->campaign_id );
    }
	...
	public function column_title( $item )
	{
		...
		return sprintf('<strong><a href="%2$s">%1$s</a> %3$s</strong>', $title, $this->get_edit_uri( $item->campaign_id ), $label );    	
	}
	...
	public function column_infos( $item )
	{
		...
		if( $item->campaign_modified !== "0000-00-00 00:00:00" ) :
			...
			if( ( $last_editor = $this->View->getDb()->meta()->get( $item->campaign_id, '_edit_last' ) ) && ( $userdata = get_userdata( $last_editor ) ) )
				$output .=		"<br>". sprintf( __( 'par %s', 'tify' ), $userdata->display_name );							
			$output .= 		"</li>";
		endif;
		$output .= "</ul>";
