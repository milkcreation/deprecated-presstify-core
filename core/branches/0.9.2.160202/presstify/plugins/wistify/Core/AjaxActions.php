...
class AjaxActions{
	...
	public function autocomplete_recipients(){
		...
		if( in_array( 'subscriber', $types ) ) :					
			if( $results = $this->master->db->subscriber->select()->rows( array( 'status' => 'registred', 's' => $term ) ) ) :
		...
		if( in_array( 'list', $types ) ) :		
			if( $results = $this->master->db->list->select()->rows( array( 's' => $term ) ) ) :
				foreach ( (array) $results as $result ){
					// Données requises
					...
					$numbers 			= $this->master->db->subscriber->select()->count( array( 'list_id' => $result->list_id, 'status' => 'registred', 'active' => 1 ) );
	...
	public function campaign_prepare(){
		...
		$this->master->db->campaign->update_status( $campaign_id, 'edit' );
		
		// Nettoyage des messages déja présent dans la file
		$this->master->db->queue->reset_campaign( $campaign_id );
		$this->master->db->campaign->set_prepare_log( $campaign_id );				
				
		// Compte le nombre d'emails à envoyer
		if( $recipients = $this->master->db->campaign->get_item_var_by_id( $campaign_id, 'recipients' ) ) :						
			/// Abonnés Wistify
			...
			if( isset( $recipients['wystify_mailing_list'] ) ) :
				...
				foreach( $recipients['wystify_mailing_list'] as $wml ) :					
					if( ! $c = $this->master->db->subscriber->select()->count( array( 'list_id' => $wml, 'status' => 'registred', 'active' => 1 ) ) )
						continue;
		...
		$this->master->db->campaign->update_prepare_log( $campaign_id, array( 'total' => $total ) );
	...
	public function campaign_prepare_subscriber(){
		foreach( $_REQUEST['subscriber_ids'] as $subscriber_id )		
			$emails[] = $this->master->db->subscriber->select()->cell_by_id( $subscriber_id, 'email' );		
		...
		// Mise à jour des logs
		$this->master->db->campaign->update_prepare_log( $_REQUEST['campaign_id'], $errors, true );
	...
	public function campaign_prepare_list(){
		$emails 		= $this->master->db->subscriber->select()->col( 'email', array( 'list_id' => $_REQUEST['list_id'], 'status'=> 'registred', 'active' => 1, 'per_page' => $_REQUEST['per_page'], 'paged' => $_REQUEST['paged'] ) );
		$processed		= count( $emails );
		$errors 		= $this->prepare_sends( $emails, $_REQUEST['campaign_id'] );
		$unprocessed 	= 0;
		foreach( $errors as $error )
	...
	public function campaign_prepare_status(){
		...
		// Mise à jour des logs
		$this->master->db->campaign->update_prepare_log( $_REQUEST['campaign_id'], array( 'enqueue' => $enqueue ), true );
			
		if( $this->master->db->queue->select()->has( 'campaign_id', $campaign_id ) ) 
			if( $this->master->db->campaign->update_status( $campaign_id, 'ready' ) )
	...
	private function prepare_sends( $emails, $campaign_id ){
		...
		foreach( (array) $emails as $email ) :
			...
			if( $queue = $this->master->db->queue->select()->row( array( 'email' => esc_attr( $email ), 'campaign_id' => $campaign_id ) ) ) :
				...
				$this->master->db->queue->handle()->record( array( 'queue_email' => esc_attr( $email ), 'queue_campaign_id' => $campaign_id, 'queue_message' => $message ) );
			endif;
		endforeach;
	...
	public function report_update(){
		...
		require_once( $this->master->admin->dir .'/ReportListTable.php' );		
		$this->table = new \tiFy\Plugins\Wistify\Admin\ReportListTable( $this->master );
		// Définition de la classe parente
		$this->table->_wp_list_table_init( array( 'screen' => $this->master->menu_slug['report'] ) );
		
		
		$report = $this->master->db->report->select()->row_by_id( $report_id );
