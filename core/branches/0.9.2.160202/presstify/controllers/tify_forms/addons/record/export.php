...
class tiFy_Forms_Addon_RecordExport{
	...
	function __construct( tiFy_Forms_Addon_Record $main ){
		...
		add_filter( 'mk_force_file_upload_allowed', array( $this, 'allowed_file_upload' ), 10, 2 );	
	}
	...
	function render(){
		...
		<form id="tify_forms-export_form" method="post" action="">
			...
			<div>
			<?php 					
				foreach( (array) $this->main->master->fields->get_list() as $slug => $field ) : if( ! $this->main->master->addons->get_field_option( 'column', 'record', $field ) ) continue; ?>
				<label>
			...
			<p>
				...
				<a href="<?php echo add_query_arg( array( 'file_upload_url' => $this->main->master->dirs->uri( 'export' ) .'/'. $filename, '_wpnonce' => wp_create_nonce( "tify_forms_record_export-" . $filename ) ), site_url() );?>" id="download-csv" class="button-secondary" style="margin-left:10px; display:none;">
