...
use tiFy\Entity\Db\Config;
use tiFy\Entity\Db\Handle;
use tiFy\Entity\Db\Make;
use tiFy\Entity\Db\Meta;
use tiFy\Entity\Db\Parse;
use tiFy\Entity\Db\Query;
use tiFy\Entity\Db\Select;
...
abstract class Db
{
	...
	// Cartographie des nom de colonnes
	...
	protected	$SearchColNames		= array();
	
	// Nom de la table de metadonnée
	protected	$MetaType			= null;
	
	// 
	protected	$PrivateQueryVars 	= array( 
	...
	protected	$AccessAttrs		= array( 'ID', 'Name', 'ColPrefix', 'ColNames', 'Primary', 'IndexKeys', 'SearchColNames', 'MetaType' );
	
	// Liste des classes Accessibles
	protected	$AccessClasses		= array( 'handle', 'meta', 'parse', 'query', 'select' );
	...
	final public function initAttrs(){
		global $tiFy;
		$attrs = $tiFy->Kernel->getEntity( $this->ID )->DbAttrs;
