<?php
/* = Déclaration = */
add_action( 'tify_taboox_register_form', 'tify_taboox_register_form_image_gallery' );
function tify_taboox_register_form_image_gallery(){
...
class tiFy_Taboox_ImageGallery extends tiFy_Taboox{	
	...
	public function item_render( $attachment_id, $name, $order ){
		...
		$output .= "\n\t<input type=\"hidden\" name=\"tify_post_meta[single][{$name}][]\" value=\"{$attachment_id}\" />";
