...
class CampaignListTable extends ListTable
{
	/* = ARGUMENTS = */
	public 	// Paramètres
			$status;
	...
	public function process_bulk_action_duplicate()
	{
		...
		check_admin_referer( $this->table->name . $this->current_action() . $item_id );

		if( ! $c = $this->table->select()->row_by_id( $item_id ) )
			return;
	...
	public function process_bulk_action_cancel()
	{
		...
		$this->table->handle()->update( $item_id, array( 'status' => 'ready' ) );
	...
	public function column_infos( $item )
	{
		...
		if( $item->campaign_modified !== "0000-00-00 00:00:00" ) :
			...
			if( ( $last_editor = $this->table->meta()->get( $item->{$this->primary_key}, '_edit_last' ) ) && ( $userdata = get_userdata( $last_editor ) ) )
				$output .=		"<br>". sprintf( __( 'par %s', 'tify' ), $userdata->display_name );							
			$output .= 		"</li>";
		endif;
		$output .= "</ul>";
