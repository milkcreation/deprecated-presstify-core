...
class Repeater extends \tiFy\Core\Field\Factory
{
    /**
     * DECLENCHEURS
     */
    ...
    {
        \wp_enqueue_style('tiFyCoreFieldRepeater');
        \wp_enqueue_script('tiFyCoreFieldRepeater');
    }
    ...
    {
        check_ajax_referer('tiFyCoreField-repeater');

        $index = self::tFyAppGetRequestVar('index', [], 'POST');
        $value = self::tFyAppGetRequestVar('value', [], 'POST');
    ...
    final protected function parseAttrs($args = [])
    {
        ...
        $defaults = [
            'before'          => '',
            'after'           => '',
            'container_id'    => 'tiFyCoreField-repeater--' . $this->getId(),
            'container_class' => '',
            'attrs'           => [],
            'name'            => '',
            'value'           => '',
            ...
            'button'       =>   [
                    ...
                    'attrs'   => [
                        'class' => 'tiFyCoreField-repeaterTrigger button-secondary'
                    ]
            ],
        ...
        if (!empty($args['container_class'])) :
            $args['container_class'] = 'tiFyCoreField-repeater ' . $args['container_class'];
        else :
            $args['container_class'] = 'tiFyCoreField-repeater';
        endif;
...
?><?php $this->before(); ?>
    ...
    >
        <?php
            Field::Button($this->getAttr('button'), true);
        ?>
    </div>
<?php $this->after(); ?><?php
    /*
