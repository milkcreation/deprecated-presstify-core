<?php
namespace tiFy\Core\Ui\Admin\Templates\Form;
...
class Form
{
    ...
    final public function current_screen($current_screen)
    {
        ...
        if (!$item_id = $this->current_item_index()) :
            if ($this->getParam('create_new_item')) :
                if (!$item_id = $this->create_new_item()) :
                    wp_die(__('Impossible de créer un nouvel élément', 'tify'));
                else :
                    wp_safe_redirect(\add_query_arg($this->getParam('item_index'), $item_id));
                    exit;
                endif;
            endif;
        endif;
    ...
    {
        \wp_enqueue_style('tiFyCoreUiAdminTemplatesForm', self::tFyAppAssetsUrl('Form.css', get_class()), [], 171115);
    }
...
<?php
    ...
    public function field_default($item, $field_name)
    {
        ...
        if ($field_name === $this->getParam('item_index')) :
            return "#{$field_value}";
        endif;
