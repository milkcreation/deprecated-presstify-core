...
class tiFy_AdminView_ListTable extends WP_List_Table{
	/* = ARGUMENTS = */
	public	// OPTIONS
			$query,
			$db,
	...
	public function parse_request(){
		...
		$per_page 	= $this->get_items_per_page( $this->db->table, $this->per_page );
	...
	public function prepare_items() {			
		...
		$this->items = $this->query->query( $this->parse_request() );
	...
	public function get_single_action_uri( $item_id, $action, $args = array(), $nonce = true ){		
		...
		if( $nonce )
			$uri = wp_nonce_url( $uri, ( is_bool( $nonce ) ? $this->db->table . $action . $item_id : $nonce ) );
	...
	private function _parse_view( $args, $index = 0 ){
		...
		$view .= ">";
		$view .= $label; 
		$view .= " <span class=\"count\">(". call_user_func( array( $this->db, 'count_items' ), $count_query_args ) .")</span>";
		$view .= "</a>";	
	...
	public function column_default( $item, $column_name ){
		...
		$col_type = strtoupper( $this->query->db->get_col_attr( $column_name, 'type' ) );
