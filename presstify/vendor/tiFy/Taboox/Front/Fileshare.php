...
class Fileshare extends Front
{
	...
	public static function Display( $post_id = null, $args = array(), $echo = true )
	{
		...
		if( ! $files = self::Get( $post->ID, $args ) )
			return;
		...
		foreach( (array) $files as $file_id ) :
			...
			$output .= "\n\t<a href=\"" . add_query_arg( array( 'file_upload_media' => $file_id, 'post_id' => $post->ID, 'fileshare_name' => $args['name'] ), site_url() ) . "\" class=\"fileshare_link clearfix\"  title=\"" . __( 'Télécharger le fichier', 'tify' ) ."\">";
			...
			$output = apply_filters( 'tify_taboox_fileshare_item', $output, $file_id );
