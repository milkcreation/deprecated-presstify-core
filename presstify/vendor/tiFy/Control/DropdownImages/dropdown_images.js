jQuery( document ).ready( function($) {	
	...
	$( document ).on( 'change', '.dropdown_images-picker.active > ul > li > ul > li > label > .selection > input[type="radio"]', function(e) {		
		...
		$selector 	= $( $(this).closest( '.dropdown_images-picker.active' ).data( 'selector' ) );	
		$picker 	= $( this ).closest( '.dropdown_images-picker.active' );
	...
	$( document ).on( 'click', '[data-tify_control="dropdown_images"]:not(.disabled) > .selected', function(e) {
		...
		$selector 	= $( $(this).closest( '[data-tify_control="dropdown_images"]' ) );
		$picker = $( $(this).closest( '[data-tify_control="dropdown_images"]' ).data( 'picker' ) );	
	...
	function getOffset( picker, input ) {
        var extraY 		= ( $('body').hasClass('wp-admin') ) ? $( '#wpadminbar' ).outerHeight() : 0;
