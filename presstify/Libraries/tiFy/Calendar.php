...
class Calendar
{
	...
	{
		echo $this->_parse_display( $_POST['date'] );
		exit;
	}	
	
	/* = PARAMETRAGE = */
	/** == Traitement de la date == **/
	private function _parse_date( $date = null ){
		if( ! $date )
	...
	/** == Pré-Traitement et Affichage == **/
	final public function _parse_display( $date = null ){
	...
	public function prev_month_button( $text = '' ){
		...
		return "<a href=\"#\" data-toggle_date=\"". $this->prev->format( 'Y-m-d' ) ."\">{$text}</a>";
	}
	...
	public function next_month_button( $text = '' ){
		...
		return "<a href=\"#\" data-toggle_date=\"". $this->next->format( 'Y-m-d' ) ."\">{$text}</a>";
	}
	
	/** == Affichage du mois courant == **/
	public function current_month( $format = null ){
		$text = ( $format === null ) ? $this->get_label( 'month', $this->selected->format( 'n' ) ) : date_i18n( $format, $this->selected->getTimestamp() ) ;
	...
	public function day_render( $day ){		
		return "<a href=\"#\" data-toggle_date=\"". $day->format( 'Y-m-d' ) ."\">". date_i18n( 'd', $day->getTimestamp() ) ."</a>";
	}    
}