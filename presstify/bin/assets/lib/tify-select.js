...
!(function ($, doc, win) {
    ...
    $.widget(
        ...
        _initStatusFlags: function () {
            ...
            this.flags.isDuplicable = (this.options.duplicate &&
                this.flags.isMultiple);
            this.flags.isRemovable = !this.flags.Disabled;
            this.flags.isSortable = ((this.options.sortable !== false) &&
                this.flags.isMultiple && !this.flags.Disabled);
        ...
        _initTrigger: function () {
            ...
            if (self.options.autocomplete) {
                ...
                self.autocompleteInput = $(
                    '<input class="tiFy-selectAutocomplete" autocomplete="off"/>').
        ...
        _initPicker: function () {
            ...
            if (self.options.picker.filter) {
                ...
                self.pickerFilter = $(
                    '<input class="tiFy-selectPickerFilter" autocomplete="off"/>').
        ...
        _remove: function (item) {
            ...
            if (!$('> [data-value="' + item.value + '"]',
                    self.selectedList).length) {
                $('> [data-value="' + item.value + '"]', self.pickerList).
        ...
        _highlight: function (item) {
            ...
            $('> [data-value="' + item.value + '"]', self.selectedList).
        ...
        _setRemovable: function (item) {
            if (this.flags.isRemovable) {
                this._get(item.index).
                    prepend('<span aria-handle="remove">×</span>');
            }
        },
        ...
        _setSortable: function (item) {
            if (!this.flags.isSortable) {
                ...
                this._get(item.index).
                    prepend('<span aria-handle="sort">...</span>');
            } else {
        ...
        _onOpen: function () {
            ...
            $(doc).
                on('click.tify_select.outside' + self.instance.uuid,
                    function (e) {
                        var close = true;
            ...
            $(win).
                on('scroll.tify_select.picker_position.' + self.instance.uuid,
                    function () {
                        var offset = self._getPickerOffset();
            ...
            self.pickerList.on('scroll.tify_select.picker_list.' +
            ...
            if (self.flags.hasFilter) {
                self.pickerFilter.on('keyup.tify_select.picker_filter.' +
                    self.instance.uuid, function () {
                    var term = $(this).val();
            ...
            // Place le curseur de saisie dans le champ de filtrage de la liste de selection
            if (self.flags.hasFilter) {
        ...
        _onClose: function () {
            ...
            $(win).
                off('scroll.tify_select.picker_position.' + self.instance.uuid);
        ...
        _onEnable: function () {
            ...
            if (self.flags.isRemovable) {
                $('> li > [aria-handle="remove"]', self.selectedList).
                    on('click.tify_select.selected_remove.' +
                        self.instance.uuid,
                        function () {
        ...
        _onDisable: function () {
            ...
            $('> li', self.pickerList).
                on('click.tify_select.picker_item_select.' +
                    self.instance.uuid);

            // Activation de la suppression d'élément dans la liste des éléments selectionnés
            if (self.flags.isRemovable) {
                $('> li > [aria-handle="remove"]', self.selectedList).
                    off('click.tify_select.selected_remove.' +
                        self.instance.uuid);
            }
        ...
        _onAdd: function (item) {
            ...
            if (item.new) {
                ...
                // Activation de la suppression d'élément dans la liste des éléments selectionnés
                if (self.flags.isRemovable) {
                    $('> [aria-handle="remove"]', self._get(item.index)).
                        on('click.tify_select.selected_remove.' +
                            self.instance.uuid, function () {
                            self._remove(item);
                        });
            ...
            self.el.trigger('tify_select.add');
        },
        ...
        _onRemove: function (item) {
            ...
            // Déclenchement des événements
            this.el.trigger('tify_select.remove');
        },
        ...
        _getPickerOffset: function () {
            ...
            if (placement === 'clever') {
                placement = ($(win).outerHeight() + $(win).scrollTop() <
                    offset.top + self.picker.outerHeight()) ? 'top' : 'bottom';
            }
        ...
        _addHandler: function (item) {
            if (!$('[data-index="' + item.index + '"]', this.handler).length) {
                $('<option value="' + item.value + '" data-index="' +
                    item.index + '" selected>' + item.label + '</option>').
