...
class ListTable extends \WP_List_Table
{
	...
	{
		// Court-circuitage de la methode magique __construct de WP_List_Table
	}
		...
		parent::__construct(
			wp_parse_args(
				...
				array(
					...
					'screen' 	=> $this->Entity->getAdminView( 'hookname' ) ? $this->Entity->getAdminView( 'hookname' ) : null
				)
			)			 
		);
	}
	...
	{
		if ( ! empty( $_REQUEST[$this->Entity->getDb()->Primary] ) ) :
			return (int) $_REQUEST[$this->Entity->getDb()->Primary];
		endif;
	...
	public function parse_request()
	{
		...
		$per_page 	= $this->get_items_per_page( $this->Entity->getDb()->Name, $this->per_page );
		...
		$args = array(						
			...
			's' 		=> $s,
			'order'		=> 'DESC',
			'orderby'	=> $this->Entity->getDb()->Primary
		);
	...
	final public function default_single_actions( $item, $action = array() )
	{
		// Edition		
		$edit 				= "<a href=\"".
									$this->get_edit_uri( $item->{$this->Entity->getDb()->Primary} )
									."\" title=\"". __( 'Éditer l\'élément', 'tify' ) ."\">". 
									__( 'Éditer', 'tify' ) 
								."</a>";		
		// Suppression définitive					
		$delete				= "<a href=\"". 
		        					$this->get_single_action_uri( $item->{$this->Entity->getDb()->Primary}, 'delete', array( '_wp_http_referer' => urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ) ) ) 
		...
		// Mise à la corbeille																				
		$trash					= "<a href=\"". 
		        					$this->get_single_action_uri( $item->{$this->Entity->getDb()->Primary}, 'trash', array( '_wp_http_referer' => urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ) ) ) 
								."\" title=\"". __( 'Mise à la corbeille de l\'élément', 'tify' ) ."\">". 
								__( 'Mettre à la corbeille', 'tify' ) 
								."</a>";
		// Sortie de la corbeille
		$untrash				= "<a href=\"". 
		        					$this->get_single_action_uri( $item->{$this->Entity->getDb()->Primary}, 'untrash', array( '_wp_http_referer' => urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ) ) ) 
								."\" title=\"". __( 'Rétablissement de l\'élément', 'tify' ) ."\">". 
								__( 'Rétablir', 'tify' ) 
								."</a>";		
		// Duplication
		$duplicate				= "<a href=\"". 
		        					$this->get_single_action_uri( $item->{$this->Entity->getDb()->Primary}, 'duplicate', array( '_wp_http_referer' => urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ) ) ) 
	...
	{				
		...
		$query = $this->Entity->getDb()->query( $this->parse_request() );
	...
	public function get_columns() 
	{
		...
		foreach( (array) $this->Entity->getDb()->col_names as $name )
			$c[$name] = $name;
		return $c;
	}
	...
	protected function process_bulk_action_delete()
	{
		...
		check_admin_referer( $this->Entity->getDb()->ID . $this->current_action() . $item_id );
		
		// Traitement de l'élément
		$this->Entity->getDb()->handle()->delete_by_id( $item_id );
		if( $this->Entity->getDb()->meta() )
			$this->Entity->getDb()->meta()->delete_all( $item_id );
	...
	protected function process_bulk_action_trash()
	{
		...
		if( $this->Entity->getDb()->meta() && ( $original_status = $this->Entity->getDb()->select()->cell( $item_id, 'status' ) ) )
			$this->Entity->getDb()->meta()->update( $item_id, '_trash_meta_status', $original_status );					
		/// Modification du statut
		$this->Entity->getDb()->handle()->update( $item_id, array( 'status' => 'trash' ) );
	...
	protected function process_bulk_action_untrash()
	{
		...
		$original_status = ( $this->Entity->getDb()->meta() && ( $_original_status = $this->Entity->getDb()->meta()->get( $item_id, '_trash_meta_status', true ) ) ) ? $_original_status : 'draft';				
		if( $this->Entity->getDb()->meta() ) $this->Entity->getDb()->meta()->delete( $item_id, '_trash_meta_status' );
		/// Mise à jour du statut
		$this->Entity->getDb()->handle()->update( $item_id, array( 'status' => $original_status ) );
	...
	{
		return esc_attr( add_query_arg( array( $this->Entity->getDb()->Primary => $item_id ), $this->Entity->getAdminView( 'menu_page_url', 'EditForm' ) ) );
	}
	
	/** == Lien vers une action appliquée à un élément == **/
	public function get_single_action_uri( $item_id, $action, $args = array(), $nonce = true )
	{		
		$args = wp_parse_args( $args, array( $this->Entity->getDb()->Primary => $item_id, 'action' => $action ) );
		$uri = add_query_arg( $args );		
		if( $nonce )
			$uri = wp_nonce_url( $uri, ( is_bool( $nonce ) ? $this->Entity->getDb()->ID . $action . $item_id : $nonce ) );
	...
	private function _parse_view( $args, $index = 0 )
	{
		$defaults = array(
			...
			'class'					=> '',
			'link_attrs'			=> array(),
			'uri'					=> $this->Entity->getAdminView( 'base_url' ), //set_url_scheme( 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ),
		...
		$view .= ">";
		$view .= $label; 
		$view .= " <span class=\"count\">(". call_user_func( array( $this->Entity->getDb()->select(), 'count' ), $count_query_args ) .")</span>";
		$view .= "</a>";	
	...
	{
		echo $this->Entity->getLabel( 'not_found' );
	}
	...
	public function column_default( $item, $column_name )
	{
		...
		$col_type = strtoupper( $this->Entity->getDb()->getColAttr( $column_name, 'type' ) );
	...
	{
        return sprintf( '<input type="checkbox" name="%1$s[]" value="%2$s" />', $this->_args['singular'], $item->{$this->Entity->getDb()->Primary} );
    }
    ...
    ?>
		<div class="wrap">
    		<h2>
    			...
    			<?php if( $this->Entity->getAdminView( 'base_url', 'EditForm' ) ) : ?>
    				<a class="add-new-h2" href="<?php echo $this->Entity->getAdminView( 'base_url', 'EditForm' );?>"><?php echo $this->Entity->getLabel( 'add_new' );?></a>
    			<?php endif;?>
    			
    			<?php if( $this->Entity->getAdminView( 'base_url', 'Import' ) ) : ?>
    				<a class="add-new-h2" href="<?php echo $this->Entity->getAdminView( 'base_url', 'Import' );?>"><?php echo $this->Entity->getLabel( 'import_items' );?></a>
    			<?php endif;?>
    		</h2>
    		...
    		<form method="post" action="<?php echo $this->Entity->getAdminView( 'base_url' );?>">
    			<?php $this->search_box( $this->Entity->getLabel( 'search_items' ), $this->EntityID );?>
    			<?php $this->display();?>
			</form>
    	</div>
    <?php
