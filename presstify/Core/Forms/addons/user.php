...
class tiFy_Forms_Addon_User extends tiFy_Forms_Addon{
	...
	public function wp_init(){		
		...
		$this->edituser_id 		= is_admin() ? ( ! empty( $_REQUEST['ID'] ) ? $_REQUEST['ID'] : 0 ) : get_current_user_id();
	...
	public function tify_taboox_register_box(){
		foreach( (array) $this->master->addons->get_forms_active( 'user' ) as $form ) :
			$this->master->forms->set_current( $form );
			$this->roles = $this->master->addons->get_form_option( 'roles', 'user' );
			...
			if( empty( $sections ) ) :
				tify_taboox_register_node(
					...
					array(
						...
						'args'	=> array( 'section' => 'default', 'form' => $this->master->forms->get_current_id() )
					)
				);		
			else :
				foreach( (array) $sections as $sect_slug => $sect_label ) :
					tify_taboox_register_node( 
						...
						array(
							...
							'args'	=> array( 'section' => sect_slug, 'form' => $this->master->forms->get_current_id() )
						)
					);
				endforeach;
			endif;
	...
	function cb_handle_check_request( &$errors, $field ){
		...
		if( ! empty( $errors ) ) :
			...
			if( is_admin() )
				...
				add_action( 'admin_notices', function() use ( $_errors ){
...
/** == == **/
class tiFy_Forms_AddonUser_NativeFields_Taboox extends tiFy\Core\Taboox\Admin{
	/* = ARGUMENTS = */
	public 	// Configuration
			$name = '',
			// Référence
...
class tiFy_Forms_AddonUser_SectionFields_Taboox extends tiFy\Core\Taboox\Admin{
	/* = ARGUMENTS = */
	public 	// Configuration
			$name = '',
			// Référence
	...
	function form( $profileuser )
	{		
		$this->main->master->forms->set_current( $this->args['form'] );
		
		$output  = "";		
		$output .= "<table class=\"form-table\">\n";
		foreach( (array) $this->main->master->fields->get_list() as $field ) :
