<?php
namespace tiFy\Core\Taboox\Admin\Post\RelatedPosts;
...
class RelatedPosts extends Admin
{
	...
	public function __construct()
	{
		...
		add_action( 'wp_ajax_tify_taboox_related_posts', array( $this, 'wp_ajax_autocomplete' ) );
	}
	...
	?>	
		<div id="tify_taboox_related_posts">
			<input 	type="text" 
					id="tify_taboox_related_posts-search-<?php echo self::$instance;?>" 
					...
					data-name="<?php echo "tify_post_meta[single][{$this->args['name']}]";?>" 
					placeholder="<?php echo $this->args['placeholder'];?>" 
					autocomplete="off" />
			<ul id="tify_taboox_related_posts-list-<?php echo self::$instance;?>" class="tify_taboox_related_posts-list thumb-list">
			<?php 
				if( $related_posts = \get_post_meta( $post->ID, $this->args['name'], true ) ) :
					foreach( $related_posts as $post_id ) :
						if ( ! $post_id ) :
							continue;
						else :
							echo $this->item_render( $post_id, "tify_post_meta[single][{$this->args['name']}]", ++$tify_taboox_related_posts_order );
						endif;
					endforeach;
				endif;
			?>	
	...
	/* = RENDU D'UN ÉLÉMENT = */
	public function item_render( $post_id, $name, $order )
	{
		$output  = "";
		$output .= "\n<li>";
	...
	public function wp_ajax_autocomplete()
	{
		...
		foreach ( $posts as $post ) {
			...
			$post->id 			= $post->ID;
			$post->render 		= 	"<a style=\"display:block; min-height:50px;\">"
									...
									."</a>";	
			$post->display 		= $this->item_render( $post->ID, $_REQUEST['name'], $_REQUEST['order'] );	
			$return[] = $post;
		}
