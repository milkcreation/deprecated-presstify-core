...
class CustomType extends Core
{
	...
	public function __construct()
	{
		...
		add_action( 'init', array( $this, 'register_taxonomy_for_object_type' ), 3 );
	...
	private function ParseTaxonomyAttrs( $taxonomy, $args = array() )
	{
		...
		// Définition des arguments du taxonomie	
		/// Intitulés
		...
		$defaults['labels'] = array();
		$defaults['labels']['singular'] = $singular;
		$defaults['labels']['plural'] 	= $plural;
		$defaults['labels']['gender'] 	= $gender;
		$labels = new \tiFy\Core\Labels\Factory( $defaults['labels'] );
		$defaults['labels'] = $labels->Get();
