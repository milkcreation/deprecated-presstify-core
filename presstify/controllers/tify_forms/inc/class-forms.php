...
Class tiFy_Forms_Forms{
	...
	public function display( $form, $echo = false ){
		...
		$output .= "\n<div id=\"tify_form_container_{$_form['ID']}\" class=\"".sprintf( $_form['container_class'], "tify_form_container" )."\">";
		// Message en cas de succès de soumission du formulaire
		$output .= "\n\t<div class=\"success notification\" style=\"display:". ( ! empty( $_REQUEST[ 'tify_forms_results-'.$_form['ID'] ] )? 'inherit' : 'none' ) ."\">";
		...
		$output .= " style=\"display:". ( ( ! empty( $_REQUEST[ 'tify_forms_results-'.$_form['ID'] ] ) && ( $success['display'] != 'form' ) ) ? 'none' : 'inherit' ) ."\">";		
		
		// Champs cachés requis 
		$output .= $this->hidden_fields( $_form );
		
		// Affichage des erreurs
		$output .= "\n\t\t<div class=\"error notification\" style=\"display:".( ( $this->master->errors->has() )? 'inherit' : 'none' )."\" >";
