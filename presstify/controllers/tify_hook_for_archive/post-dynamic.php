...
class tiFy_HookForArchive_PostDynamic extends tiFy_HookForArchive_Post{
	...
	function __construct( tiFy_HookForArchive_Master $master ){
		...
		add_filter( 'post_type_link', array( $this, 'wp_post_type_link' ), 10, 4 );
		...
		add_filter( 'mktzr_breadcrumb_is_singular', array( $this, 'tify_breadcrumb_is_singular' ), 10, 5 );
		add_filter( 'mktzr_breadcrumb_is_post_type_archive', array( $this, 'tify_breadcrumb_is_post_type_archive' ), 10, 2 );
	}
	...
	function tify_taboox_register_node(){
		foreach( $this->get_hooks() as $archive_post_type => $v ) 
			...
			if( $v['taboox_auto'] )
				tify_taboox_register_node( $archive_post_type, array( 
...
class tiFy_HookForArchive_Taboox extends tiFy_Taboox{
	...
	function form( $_args = array() ){
		...
		$value = ( $_value = tify_get_post_meta_multi( $this->post->ID, '_'. $name ) ) ? $_value : array();
