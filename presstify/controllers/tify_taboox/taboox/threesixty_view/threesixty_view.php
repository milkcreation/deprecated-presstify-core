 ...
 */
class tiFy_Taboox_ThreeSixtyView extends tiFy_Taboox{
	...
	public function current_screen( $screen ){
		...
		wp_enqueue_style( 'tify_taboox_threesixty_view', $this->uri .'/admin.css', array(), 150907 );
		wp_enqueue_script( 'tify_taboox_threesixty_view', $this->uri .'/admin.js', array( 'jquery' ), 150907, true );
	...
	public function enqueue_scripts(){
		...
		wp_enqueue_style( 'tify_taboox_threesixty_view' );
		wp_enqueue_script( 'tify_taboox_threesixty_view' );
	...
	<div id="threesixty_view-<?php echo $this->instance;?>" class="threesixty_view-taboox">
		<input type="hidden" name="tify_post_meta[single][threesixty_view]" value="" />
		...
		<?php 	
			if( ! empty( $this->value ) ) :
				...
				foreach( $thumbs as $thumb ) :
					if( is_array( $thumb ) )
						echo threesixty_view_item( array( 'post_id' => $this->post->ID, 'attachment_id' => $thumb['attachment'], 'name' => 'tify_post_meta[single][threesixty_view]', 'order' => $thumb['order'] ) );
				endforeach;
	...
	function wp_ajax(){
		echo json_encode( array(
				'html' 	=> threesixty_view_item( array( 'post_id' => $_POST['post_id'], 'attachment_id' => $_POST['attachment_id'], 'name' => "tify_post_meta[single][{$this->name}]", 'order' =>  $_POST['order'] ) ),
				'order'	=> $_POST['order']
			)
		);		 
		exit;
