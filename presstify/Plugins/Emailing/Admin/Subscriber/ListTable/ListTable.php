...
class ListTable extends tiFy_ListTable
{
	...
	public function column_subscriber_email( $item )
	{
		...
		$label = ( \tify_db_get( 'wistify_list_relationships' )->is_orphan( $item->subscriber_id ) ) ? "<span> - ". __( 'Orphelin', 'tify' ) ."</span>" : false;
		
		return sprintf('<strong><a href="%2$s">%1$s</a> %3$s</strong>', $title, $this->get_edit_uri( $item->subscriber_id ), $label );     	
	}

	/** == Contenu personnalisé : Listes de diffusion == **/
	public function column_subscriber_lists( $item )
	{
		$DbList 	= \tify_db_get( 'wistify_list' );
		$DbListRel 	= \tify_db_get( 'wistify_list_relationships' );
		...
		if( $list_ids = $DbListRel->select()->col( 'list_id', array( 'subscriber_id' => $item->subscriber_id, 'active' => 1 ) ) ) : 
			...
			foreach( $list_ids as $list_id )
				$list[] = "<a href=\"". ( add_query_arg( 'list_id', $list_id, $this->base_url ) ) ."\">". ( $list_id ? $DbList->select()->cell_by_id( $list_id, 'title' ) : __( 'Inscription sans liste', 'tify' ) )  ."</a>";
			$output .= join( ', ', $list );
		else :
		...
		if( $list_ids = $DbList->get_subscriber_list_ids( $item->subscriber_id, 0 ) ) : 
			$list = array();
			foreach( $list_ids as $list_id )
				$list[] = "<a href=\"". ( add_query_arg( 'list_id', $list_id, $this->base_url ) ) ."\">". $DbList->select()->cell_by_id( $list_id, 'title' ) ."</a>";
			$output .= join( ', ', $list );
		else :
			$output .=  __( 'Aucune', 'tify' );
		endif;
