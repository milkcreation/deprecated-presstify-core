...
class EditForm extends tiFy_EditForm
{
	...
	final public function admin_enqueue_scripts()
	{
		...
		wp_enqueue_style( 'tify_wistify_campaign', $this->Url .'/campaign-edit.css', array( ), '150403' );
		...
		switch( $this->current_step ) :
			case 1 :
				...
				break;
			case 2 :
				wp_enqueue_script( 'tify_wistify_campaign-step2', $this->Url .'/campaign-edit-step2.js', array( 'jquery' ), '150928', true );
				// Actions et Filtres Wordpress
				add_filter( 'tiny_mce_before_init', array( $this, 'tiny_mce_before_init' ), 99, 2 );
				break;
			case 3 :
				wp_enqueue_style( 'tify_wistify_campaign-step3', $this->Url .'/campaign-edit-step3.css', array( 'tify_control-suggest' ), '150918' );
				wp_enqueue_script( 'tify_wistify_campaign-step3', $this->Url.'/campaign-edit-step3.js', array( 'jquery', 'tify_control-suggest' ), '150918', true );
				break;
			case 4 :						
				tify_control_enqueue( 'switch' );
			break;
			case 5 :
				wp_enqueue_style( 'tify_wistify_campaign-step5', $this->Url .'/campaign-edit-step5.css', array( 'tify_control-touch_time' ), '150918' );
				wp_enqueue_script( 'tify_wistify_campaign-step5', $this->Url .'/campaign-edit-step5.js', array( 'jquery', 'tify_control-touch_time' ), '150918', true );
	...
	?>	
		...
		<?php foreach( range( 1, 5, 1 ) as $step ) :?>
			...
			<?php if( ( $step <= $this->item->campaign_step ) && ( $step != $this->current_step ) ) :?>
				<a href="<?php echo add_query_arg( array( $this->View->getDb()->Primary => $this->item->campaign_id, 'step' => $step ), $this->base_url );?>"><?php echo $step_txt;?></a>
			<?php else :?>
	...
	public function step_2( )
	{
		...
		add_filter( 'mce_css', create_function( '$mce', 'return "'. $this->Url . '/css/editor-style.css";' ) );
	...
	public function step_3( )
	{
		...
		$DbSubcriber 	= \tiFy\Core\Db\Db::Get( 'wistify_subscriber' );
		$DbList 		= \tiFy\Core\Db\Db::Get( 'wistify_list' );
	?>
	...
	{
		$DbSubcriber 	= \tiFy\Core\Db\Db::Get( 'wistify_subscriber' );
		$DbMailQueue 	= \tiFy\Core\Db\Db::Get( 'wistify_queue' );
	...
	public function minor_actions(){?>
		<?php if( $this->current_step === 5 ) : $set_send_active = $this->master->db->queue->select()->has( 'campaign_id', $this->item->campaign_id );?>
		...
		<div class="nav">
			...
			<a href="<?php echo add_query_arg( array( 'step' => $this->current_step-1, $this->View->getDb()->Primary => $this->item->campaign_id ), $this->base_url );?>" class="prev button-secondary"><?php _e( 'Étape précédente', 'tify' );?></a>
			<?php endif;?>
			<?php if( $this->item->campaign_step > $this->current_step ) :?>
			<a href="<?php echo add_query_arg( array( 'step' => $this->current_step+1, $this->View->getDb()->Primary => $this->item->campaign_id ), $this->base_url );?>" class="next button-secondary"><?php _e( 'Étape suivante', 'tify' );?></a>
			<?php endif;?>
		</div>
		<?php endif;?>
	<?php
			...
			<a href="<?php echo wp_nonce_url( 
	        					add_query_arg( 
        							array( 
        								'page' 				=> $_REQUEST['page'], 
        								'action' 			=> 'trash', 
        								$this->View->getDb()->Primary 			=> $this->item->{$this->db->primary_key}
									),
									admin_url( 'admin.php' ) 
								),
	...
	{			
		$this->item = $this->View->getDb()->select()->row_by_id( (int) $_GET[$this->View->getDb()->Primary] );
		...
		if( ! isset( $_GET['step'] ) ) :
			$sendback = add_query_arg( array( $this->View->getDb()->Primary => $this->item->campaign_id, 'step' => $this->item->campaign_step ), $this->base_url );		
