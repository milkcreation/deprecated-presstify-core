<?php
class tiFy_WebAgencyCRM_AdminTeam{
		/* = ARGUMENTS = */
	...
	/* = CONSTRUCTEUR = */
	function __construct( tiFy_WebAgencyCRM_Master $master ){
	...
	public function wp_admin_menu(){
		// Configuration				
		$this->menu_slug 		= $this->master->menu_slug['team'];
		$this->hook_suffix 		= $this->master->hookname['team'];
	...
	public function wp_current_screen(){
		// Bypass
		if( get_current_screen()->id !== $this->master->hookname['team'] ) 
			return;		
		...
		switch( @$request['action'] ) :
			default :
			case 'list' :
				require_once( $this->master->admin->dir .'/inc/team-table.php' );
				$this->list_table = new tiFy_WebAgencyCRM_AdminListTeam( $this->master );
				break;
			case 'edit' :
				require_once( $this->master->admin->dir .'/inc/team-edit.php' );
				$this->edit_form = new tiFy_WebAgencyCRM_AdminEditTeam( $this->master );		
				break;
		endswitch;
	}
