...
class MailingListListTable extends ListTable
{
	/* = ARGUMENTS = */
	public 	// Paramètres
			$status;
	...
	public function process_bulk_action_delete()
	{
		...
		$this->master->db->list_rel->delete_list_subscribers( $item_id );
	...
	{
		$registred 		= (int) $this->master->db->subscriber->select()->count( array( 'list_id' => $item->list_id, 'status' => 'registred', 'active' => 1 ) );
		$unsubscribed 	= (int) $this->master->db->subscriber->select()->count( array( 'list_id' => $item->list_id, 'status' => 'registred', 'active' => 0 ) );
		$waiting 		= (int) $this->master->db->subscriber->select()->count( array( 'list_id' => $item->list_id, 'status' => 'registred', 'active' => -1 ) );
		$trashed 		= (int) $this->master->db->subscriber->select()->count( array( 'list_id' => $item->list_id, 'status' => 'trash' ) );			
		$total 			= (int) $this->master->db->subscriber->select()->count( array( 'list_id' => $item->list_id ) );
