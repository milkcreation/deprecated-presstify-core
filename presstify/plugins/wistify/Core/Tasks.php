...
class Tasks{
	...
	public function shedule_queue_handle(){
		...
		foreach( $campaign_ids as $campaign_id ) :
			...
			if( 0 === $this->master->db->queue->select()->count( array( 'campaign_id' => $campaign_id ) ) ) :
				$this->master->db->campaign->update_status( $campaign_id, 'forwarded' );
				continue;
			endif;	
			...
			for( $count; $count <= $max; $count++ ) :
				...
				if( ! $q = $this->master->db->queue->select()->row( array( 'campaign_id' => $campaign_id, 'locked' => 0, 'orderby' => 'id', 'order' => 'ASC' ) ) )
					break;
				
				// Verrouillage du message à envoyer
				$this->master->db->queue->handle()->update( $q->queue_id, array( 'locked' => 1 ) );	
				
				// Tentative d'envoi du message
				$result = $this->master->Mandrill->result( 'messages', 'send', array( $q->queue_message ) );
				if( is_wp_error( $result ) ) :
					$this->master->db->queue->handle()->update( $q->queue_id, array( 'locked' => 0 ) );
					continue;
				endif;
				$resp = current( $result );

				// Sauvegarde du rapport
				$this->master->db->report->handle()->record( 
					array(
				...
				$this->master->db->queue->handle()->delete_by_id( $q->queue_id );
			endfor;
	...
	public function mail( $campaign_id, $to, $args = array( ) ){
		if( ! $campaign = $this->master->db->campaign->select()->row_by_id( $campaign_id ) )
			return;		
	...
	public function prepare_mail( $campaign_id, $to, $args = array() ){
		$campaign = $this->master->db->campaign->select()->row_by_id( $campaign_id );
		...
		elseif( $u = $this->master->db->subscriber->select()->row_by( 'email', $to ) ) :
