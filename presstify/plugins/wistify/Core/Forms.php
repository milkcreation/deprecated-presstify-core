...
class tiFy_Wistify_Forms_Addon extends \tiFy_Forms_Addon{
	/* = ARGUMENTS = */
	public	// Paramètres
			$list_id = 0,
			$subscriber_id = 0;
	...
	public function cb_handle_check_request( &$errors, $field ){
		...
		if( $wistify->db->subscriber->select()->count( array( 'email' => $field['value'], 'status' => 'trash' ) ) )
		...
		if( $wistify->db->subscriber->select()->count( array( 'email' => $field['value'], 'list_id' => $this->list_id, 'active' => -1 ) ) )
	...
	public function cb_handle_submit_request( &$parsed_request, $original_request ){
		...
		if( ! $subscriber = $wistify->db->subscriber->select()->row( array( 'email' => $email ) ) ) :
			$data = array( 
				'subscriber_uid'		=> tify_generate_token( 32 ),
				'subscriber_email'		=> $email,
				'subscriber_date'		=> current_time( 'mysql' )
			);
			$this->subscriber_id = (int) $wistify->db->subscriber->handle()->record( $data );
		else :
			$this->subscriber_id = (int) $subscriber->subscriber_id;
		endif;
		
		$wistify->db->list_rel->insert_subscriber_for_list( (int) $this->subscriber_id, (int) $this->list_id, -1 );
	}
	...
	public function cb_handle_before_redirect( &$parsed_request, $original_request ){
		...
		$subscriber_uid = $wistify->db->subscriber->select()->cell_by_id( $this->subscriber_id, 'uid' );
		$list_uid		= $wistify->db->list->select()->cell_by_id( $this->list_id, 'uid' );
		...
		$tiFy_Mailer = new tiFy_Mailer( 
			array( 
				...
				'to'   		=> array( 'email' => $wistify->db->subscriber->select()->cell_by_id( $this->subscriber_id, 'email' ) ),
