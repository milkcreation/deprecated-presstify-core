<?php
class tiFy_Wistify_AdminEditCampaign extends tiFy_AdminView_EditForm{
	/* = ARGUMENTS = */	
	public	$current_step;
	...
	public function process_bulk_action_edit(){		
		if( ! $this->item->{$this->primary_key} ) :				
				$this->get_default_item_to_edit();
				
				// Traitement de la redirection
				$sendback = add_query_arg( array( $this->primary_key => $this->item->{$this->primary_key} ), $this->base_url );		
			...
			if( ! isset( $_GET['step'] ) ) :
				$sendback = add_query_arg( array( $this->primary_key => $this->item->{$this->primary_key}, 'step' => $item->campaign_step ), $this->base_url );		
				wp_redirect( $sendback );
				exit;
