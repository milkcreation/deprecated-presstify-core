...
class SubscriberListTable extends ListTable
{
	...
	public function __construct( Wistify $master )
	{
		...
		$this->prepare_query_args = array(
			'status' 	=> ! empty( $_REQUEST['status'] ) 	? $_REQUEST['status'] 	: array( 'registred', 'waiting', 'unsubscribed' ),
	...
	public function column_subscriber_lists( $item )
	{
		...
		if( $list_ids = $this->master->db->list_rel->select()->col( 'list_id', array( 'subscriber_id' => $item->{$this->primary_key}, 'active' => 1 ) ) ) : 
			...
			foreach( $list_ids as $list_id )
				$list[] = "<a href=\"". ( add_query_arg( 'list_id', $list_id, $this->base_link ) ) ."\">". ( $list_id ? $this->master->db->list->select()->cell_by_id( $list_id, 'title' ) : __( 'Inscription sans liste', 'tify' ) )  ."</a>";
			$output .= join( ', ', $list );
		else :
		...
		if( $list_ids = $this->master->db->list->get_subscriber_list_ids( $item->{$this->primary_key}, 0 ) ) : 
			$list = array();
			foreach( $list_ids as $list_id )
				$list[] = "<a href=\"". ( add_query_arg( 'list_id', $list_id, $this->base_link ) ) ."\">". $this->master->db->list->select()->cell_by_id( $list_id, 'title' ) ."</a>";
			$output .= join( ', ', $list );
		else :
			$output .=  __( 'Aucune', 'tify' );
		endif;
