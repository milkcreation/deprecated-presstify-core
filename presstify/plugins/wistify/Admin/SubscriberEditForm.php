...
class SubscriberEditForm extends EditForm
{
	/* = ARGUMENTS = */	
	private	// Référence
			$master;
	...
	public function parse_postdata( $data )
	{
		...
		elseif( $this->table->select()->has( 'email', $data['subscriber_email'], array( 'item__not_in' => $data['subscriber_id'] ) ) )
	...
	protected function process_bulk_action_update()
	{
		...
		if( is_wp_error( $data ) ) :
			...
			$subscriber_id = $this->table->handle()->record( $data );					
			// Enregistrement des liaisons abonné/liste
			/// Récupération des listes liées à l'abonné
			$original_lists = $this->master->db->list->get_subscriber_list_ids( $subscriber_id );	
			...
			if( empty( $update_lists ) )
				$this->master->db->list_rel->insert_subscriber_for_list( (int) $subscriber_id, 0, 1 );
			else
				$this->master->db->list_rel->delete_subscriber_for_list( (int) $subscriber_id, 0 );
							
			//// Suppression des anciennes listes
			foreach( array_diff( $original_lists, $update_lists ) as $list_id )
				 $this->master->db->list_rel->delete_subscriber_for_list( (int) $subscriber_id, (int) $list_id );
			//// Ajout des nouvelles listes
			foreach( (array) $update_lists as $list_id )
				$this->master->db->list_rel->insert_subscriber_for_list( (int) $subscriber_id, (int) $list_id, 1 );
	...
	{
		$suscriber_list = $this->master->db->list->get_subscriber_list_ids( $this->item->subscriber_id );
	?>
		...
		<?php if( $this->master->db->list_rel->is_orphan( $this->item->subscriber_id ) ) :?>
		<div style="color:red;"><?php _e( 'Cet abonné est actuellement affilié à la liste des orphelins (inscription sans liste).', 'tify' );?></div>
		<?php endif;?>
		<ul id="mailing-lists">
		<?php foreach( (array) $this->master->db->list->select()->rows( array( 'orderby' => 'title', 'order' => 'ASC', 'status' => 'publish' ) ) as $l ) : $checked = in_array( $l->list_id, $suscriber_list ) ? true : false; ?>
			<li>
				<label>
					...
					<span class="numbers"><?php echo $this->master->db->list->select()->count( array( 'list_id' => $l->list_id, 'status' => 'registred', 'active' => 1 ) );?></span>
				</label>
			</li>
		<?php endforeach;?>
		</ul>
	...
	?>
		<div class="deleting">			
			<a href="<?php echo wp_nonce_url( 
									...
									'wistify_subscriber_trash_'. $this->item->{$this->primary_key} 
