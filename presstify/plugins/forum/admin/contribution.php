<?php
class tiFy_Forum_AdminContribution{
		/* = ARGUMENTS = */
	...
	public function wp_admin_menu(){
		// Configuration				
		$this->menu_slug 		= $this->master->menu_slug['contribution'];
		$this->hook_suffix 		= $this->master->hookname['contribution'];
	...
	public function wp_current_screen(){
		// Bypass
		if( get_current_screen()->id !== $this->master->hookname['contribution'] ) 
			return;		
		...
		switch( @$request['action'] ) :
			default :
			case 'list' :
				require_once( $this->master->admin->dir .'/inc/contribution-table.php' );
				$this->list_table = new tiFy_Forum_AdminListContribution( $this->master );
				break;
			case 'edit' :
				require_once( $this->master->admin->dir .'/inc/contribution-edit.php' );
				$this->edit_form = new tiFy_Forum_AdminEditContribution( $this->master );		
				break;
		endswitch;
	}
	...
	?>
		<div class="wrap">
			<h2>
				<?php _e( 'Contributions', 'tify' );?>
				<a class="add-new-h2" href="<?php echo $this->edit_link;?>"><?php _e( 'Ajouter une contribution', 'tify' );?></a>
			</h2>
			...
			<form method="get">
				...
				<?php $this->list_table->search_box( __( 'Recherche de contribution' ), 'tify_forum_contribution' );?>
	...
	?>
		<div class="wrap">
			<h2>
				<?php _e( 'Éditer une contribution', 'tify' );?>
				<a class="add-new-h2" href="<?php echo $this->edit_link;?>"><?php _e( 'Ajouter une autre contribution', 'tify' );?></a>
			</h2>
