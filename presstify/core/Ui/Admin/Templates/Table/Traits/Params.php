...
trait Params
{
     ...
     *
     * @param array $filtered_views Liste des vues filtrées définies en paramètre
     *
     * @return array
     */
    public function set_param_filtered_views($filtered_views = [])
    {
    ...
    /**
     * Définition de l'activation d'ajout automatique des actions sur un élément dans les cellules de la colonne principale
     *
    ...
    /**
     * Définition de la liste des données passées en argument dans la requête ajax de prévisualisation d'un élément
     *
     * @param array $preview_item_ajax_datas Liste des données passées en argument dans la requête ajax de prévisualisation d'un élément définies en paramètre
     *
     * @return array
     */
    public function set_param_preview_item_ajax_datas($preview_item_ajax_datas = [])
    {
        return $preview_item_ajax_datas;
    }
     ...
     *
     * @param array $notices Liste des des vues filtrées existantes
     *
     * @return array
     */
    public function init_param_filtered_views($filtered_views = [])
    {
        if ($filtered_views) :
            $filtered_views = $this->parseViews($filtered_views);
        endif;
    ...
    /**
     * Initialisation de la liste des données passées en argument dans la requête ajax de prévisualisation d'un élément
     *
     * @param array $preview_item_ajax_datas Liste des données passées en argument dans la requête ajax de prévisualisation d'un élément existantes
     *
     * @return array
     */
    public function init_param_preview_ajax_datas($preview_item_ajax_datas = [])
    {
        if(!isset($preview_item_ajax_datas['_ajax_nonce'])) :
            $preview_item_ajax_datas['_ajax_nonce'] = \wp_create_nonce('tiFyCoreUiAdminTemplatesTablePreviewItem');
        endif;
