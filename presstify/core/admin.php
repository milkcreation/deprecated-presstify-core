...
class tiFy_CoreAdmin{
	...
	public function __construct( tiFy $master ){
		...
		require_once $this->master->get_directory(). '/admin/plugins.php';
	...
	final public function wp_admin_menu(){
	  	add_menu_page( __( 'PresstiFy', 'tify' ) , __( 'PresstiFy', 'tify' ), $this->master->capabilities->cap, 'tify', null, null, 66 );
	}
	
	/** == Personnalisation du pied de page de l'interface d'administration == **/
	final public function wp_admin_footer_text( $text = '' ){
		$admin_footer_text = $this->master->get_config( 'admin_footer_text' );
	...
	final public function wp_admin_bar_menu( $wp_admin_bar ){
		if( ! $admin_bar_menu_logo = $this->master->get_config( 'admin_bar_menu_logo' ) )
			return;
