...
/**
 * @name TakeOver
 * @desc Prise de contrôle d'un compte utilisateur
 * @package presstiFy
 * @namespace tiFy\Core\Users\Factory\TakeOver
 * @version 1.1
 * @subpackage Core
 * @since 1.2.535
 *
...
class TakeOver extends \tiFy\App\FactoryConstructor
{
    /**
    ...
    public function __construct($id, $attrs = [])
    {
        ...
        $this->Emitter->addListener('tiFy.Core.Users.TakeOver.canSwitch.' . $this->getId(), [$this, 'eventCanSwitch']);
     ...
     * @see \tiFy\Core\Users\Factory\TakeOver\TakeOver::canSwitch
     *
    ...
    {
        if ($event->getName() !== 'tiFy.Core.Users.TakeOver.canSwitch.' . $this->getId()) :
    ...
    private function _canSwitch($user)
    {
        ...
        elseif (!$this->isAllowed($user)) :
            ...
            $this->Emitter->emit('tiFy.Core.Users.TakeOver.canSwitch.'. $this->getId(), $caller, $called);
        endif;
