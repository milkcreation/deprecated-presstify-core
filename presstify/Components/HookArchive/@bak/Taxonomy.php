...
{
	/* = ARGUMENTS = */
	protected $RegisteredTaxonomy	= array();
	...
	{	
		...
		foreach( (array) $this->GetHooks() as $hook ) :
			...
			var_dump( get_post_type( $hook['id'] ) );
			
			add_rewrite_rule( "{$archive_slug}/?$", "index.php?{$taxonomy}={$term->slug}&tify_hook_id={$hook['id']}", 'top' );
			add_rewrite_rule( "{$archive_slug}/{$wp_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?{$taxonomy}={$term->slug}&tify_hook_id={$hook['id']}" . '&paged=$matches[1]', 'top' );			
		endforeach;
		exit;
